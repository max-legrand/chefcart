...
type LoginController interface {
	...
	LoginTest(username, password *Tokens.Token) string
}
...
func (controller *loginController) LoginTest(username, password *Tokens.Token) string {
	// Gets login form information and binds to struct
	var credential LoginCredentials
	credential.Email = username.Token
	credential.Password = password.Token
	// Hash and update password
	data := []byte(credential.Password)
	hash := md5.Sum(data)
	credential.Password = hex.EncodeToString(hash[:])
